...
```
(0,005 ГБ * 13М) ГБ / 1024 / 1024 ≈ 63,5 Тб
```
...
| Данные                                             | Размер     |
|:---------------------------------------------------|:----------:|
| Хранение данных запросов на всех ДЦ в сумме        |  63,5 Пб   |
...
В каждом ДЦ будет стоять по 2 node, в каждой из них будет поднята БД, одна из них будет master, друга репликой. Обе ноды будут обращаться в одну БД, пока та не станет не доступной.
...
В таблице queries необходимо сделать поле username индексом, потому что часто будет сбор запросов с конкретным пользователем.
