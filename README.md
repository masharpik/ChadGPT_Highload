...
В среднем поддержим RPS 8 запросов/с на каждом ДЦ.
...
С учетом, что на хранение генерации изображений уйдет 6200 Тб вместе с текстом запроса, то на хранение изображений в ответ на запросы уйдет:
```
6200 - 1024б /1024/1024/1024/1024 * 2_000_000 * 30*12*3 ≈ 6198 Тб
```

Хранение авторизационных токенов на 1 ДЦ при размере одного токена до 8 КБ, 20 ДЦ и 180,5 М активных пользователей в месяц с учетом жизни токена месяц будет равен:
```
180,5 M * 8 КБ / 20 ≈ 70 ГБ
```

| Данные                                             | Размер     |
|:---------------------------------------------------|:----------:|
| Хранение данных запросов на всех ДЦ в сумме        |  63,5 Тб   |
| Хранение изображений на всех хранилищах ДЦ в сумме |  6198 Тб   |
| Вес моделей на каждой из 20 ДЦ	                 | 700 Гб     |
| Вес дополнительных данных для обучения на 1 ДЦ     | 6_840 ГБ   |
| Хранение авторизационных токенов на 1 ДЦ           | 70 ГБ      |
...
Шардинг будет по имени username в таблицах users и queries. Для шардинга будет использоваться stateless-posrtgres-query-request.

#### Индексы

У таблицы users и queries будет по уникальному индексу на столбец username. Размер индексов обычно незначителен для расчета ресурсов.
...
| Ngnix         | Резервирование | Необходимо иметь два ngnix в каждом ДЦ, каждый будет на одинаковом ip-адресе, второй ngnix будет подменять при отказе первого ngnix |
...
| Redis | Резервирование | Redis используется для авторизации пользователя, в случае, если откажет одна БД, ее подменит поднятая вторая БД. Репликация требуется |
...
### 10. Алгоритмы <a id="11"></a>

#### Алгоритм создания языковой модели ChatGPT:

1) Получение данных для обучения
WhatsApp и Telegram будут являться источником для передачи разговорного стиля. Для разнообразия можно взять аккаунты людей из сфер деятильности: учитель, студент-отличник, программист, филосов.
Данные сайтов будут являться источником для передачи научных данных.

Данные можно форматировать в (`Дата`, `Имя автора`, `Текст данных`).

2) Токенизация
Чтобы обучить языковую модель, нужно разбить язык на части (так называемые токены) и постепенно подавать их в модель. Токенизация может быть выполнена на уровене слов. Текст представляется как последовательность слов. Однако словарный запас модели ограничен существующими словами в обучающих данных.

Поскольку многие слова встречаются только один или два раза, заменим такие редкие слова специальным токеном “<UNK>”. В результате словарный запас сократится, что приведет к уменьшению модели, которую нужно будет обучать позже.
...
Применим архитектуру GPT, которая была популяризирована в статье [“Все, что вам нужно,  —  это внимание” (“Attention is All you Need”)](https://arxiv.org/abs/1706.03762). Поскольку мы создаем бота, отвечающего на вопросы, нам необходима архитектура трансформера (в левой части).

![gpt architecture](img/gpt_architecture.png)

5) Обучение модели
Для обучения языковой модели были выбраны последовательности токенов из случайных мест в обучающих данных. Учитывая быстрый темп разговоров в социальных сетях, длины контекста в 32 слова будет достаточно. Таким образом, были выбраны случайные фрагменты по 32 слова в качестве входа контекста и использованы соответствующие векторы, сдвинутые на одно слово, в качестве целей для сравнения.
...
6) Режим работы чата
Модель завершает сгенерированное сообщение, когда предсказывает, что следующим будет токен <END>.

### 11. Расчет ресурсов <a id="12"></a>

*Количество серверов* – 20.

Так как на хранение данных в БД ≈ 64 Тб, каждую БД возьмем по 10 ТБ SSD и 256 RAM. *Количество экземпляров Postgres* – 7.

На хранение изображений в s3 требуется ≈ 12,5 Пб = 13107200 ГБ.

На каждом сервере будет настроено расположение следующих сервисов:
| Название сервиса | Кол-во экземпляров | Требование к одному экземпляру | Минимальная конфигурация | Дополнение |
| :--- | :---: | :---: | :---: | :---: |
| Ngnix | 2 | 8 RPS | 2 CPU | 2 CPU поддердит до 9 RPS для HTTPS запросов при 100 КБ передаваемых данных |
| Application instance | 2 | 8 RPS | 4 CPU | Нам нет необходимости хранить данные, так как это stateless-сервер |
| Worker | 2 | 4 RPS | 4 CPU, 350 RAM, 700 SSD | Объем модели 700 ГБ, однако виртуальная ОП позволяет уменьшить объем до 2 раз без потери производительности |
| Redis | 2 | 4 RPS | 1 CPU, 70 RAM, 70 SSD | Так как redis хранит и обрабатывает данные в ОП, необходимо 70 RAM |
| PostgreSQL | 7 | 10 Тб | - | PostgreSQL будет арендовано у amazon, так что от нас скрыта реализация под капотом |
| S3 | - | 6_346_752 ГБ | - | S3 будет арендовано у amazon, так что от нас скрыта реализация под капотом |

| Аренда | Стоимость в месяц |
| :--- | :---: |
| Ngnix | 12,82 USD * 20 серверов = 256,40 USD |
| Application instance | 45,10 USD * 20 = 90,20 USD |
| Worker | 2_178,08 USD * 20 = 43561,60 USD |
| Redis | 476,45 USD * 20 = 9529,00 USD |
| PostgreSQL | 320_000,00 USD * 7 = 2_240_000,00 USD |
| s3 | 133_845,00 USD |
| Итого | 2_427_282,20 USD |
